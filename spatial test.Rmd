...
ggplot(DC_data) + 
  geom_sf(aes(fill = `Race_White alone`),size=.1) +
  ...
  scale_fill_continuous(name = "Race White Proportion", labels = comma, high = "blue", low = "lightgrey") +
  labs(title = "Distribution of Bike Stations by Census Tract") +
  theme_classic() + 
...
```{r}
#Distance to Point of interests

bike_spatial$distToWM = st_distance(bike_spatial,points_of_interest %>% filter(ALIASNAME == "WASHINGTON MONUMENT")) %>% as.vector()

bike_spatial$distToGU = st_distance(bike_spatial,points_of_interest %>% filter(ALIASNAME == "GEORGE WASHINGTON UNIVERSITY HALL OF GOVERNMENT")) %>% as.vector()

bike_spatial$distToWF = st_distance(bike_spatial,points_of_interest %>% filter(ALIASNAME == "THE YARDS PARK WATER FEATURE")) %>% as.vector()

bike_spatial$distToDF = st_distance(bike_spatial,points_of_interest %>% filter(ALIASNAME == "DUPONT MEMORIAL FOUNTAIN")) %>% as.vector()


# bike_spatial <- bike_spatial %>% 
#   filter(GEOID != 11001980000)
```
